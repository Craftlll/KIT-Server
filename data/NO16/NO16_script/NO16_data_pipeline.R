# -----------------------------------------------------------------------------
# 脚本名称: NO16_data_pipeline.R
# 功能: 数据预处理与轻量化转换流水线 (NO16 Single 10x Folder)
# -----------------------------------------------------------------------------
...
# ==============================================================================
# 模块 A: 预处理 (Single 10x Folder -> RDS)
# ==============================================================================
run_preprocess <- function(input_dir, output_rds) {
    ...
    # NO16 has 10x files directly in input_dir
    # Check for matrix.mtx.gz
    if (!file.exists(file.path(input_dir, "matrix.mtx.gz"))) {
        stop("matrix.mtx.gz not found in input directory!")
    }

    tryCatch(
        {
            s_name <- "NO16_Single10x"
            flog.info("Read10X failed or using manual fallback. Loading files manually...")
            mat_path <- file.path(input_dir, "matrix.mtx.gz")
            feat_path <- file.path(input_dir, "features.tsv.gz")
            bar_path <- file.path(input_dir, "barcodes.tsv.gz")

            counts <- Matrix::readMM(mat_path)
            flog.info(paste("Matrix dimensions:", paste(dim(counts), collapse = " x ")))

            features <- fread(feat_path, header = FALSE)$V1
            barcodes <- fread(bar_path, header = FALSE)$V1

            flog.info(paste("Features loaded:", length(features)))
            flog.info(paste("Barcodes loaded:", length(barcodes)))

            if (length(features) != nrow(counts)) stop("Feature count mismatch!")
            if (length(barcodes) != ncol(counts)) stop("Barcode count mismatch!")

            # Deduplicate features
            if (any(duplicated(features))) {
                flog.warn("Duplicate features found - resolving...")
            ...
            flog.info("Object created.")

            # Check for MT/mt
            flog.info("Calculating MT percentage...")
            obj[["percent.mt"]] <- PercentageFeatureSet(obj, pattern = "^MT-")

            # Debug values
            mt_mean <- mean(obj$percent.mt)
            flog.info(paste("Mean MT (Upper):", mt_mean))

            if (mt_mean < 0.1) {
                flog.info("Checking lowercase mt-...")
                obj[["percent.mt"]] <- PercentageFeatureSet(obj, pattern = "^mt-")
                flog.info(paste("Mean MT (Lower):", mean(obj$percent.mt)))
            }
...
if (length(args) < 1) stop("Usage: Rscript NO16_data_pipeline.R [preprocess|convert] ...")
action <- args[1]
input_val <- NULL
output_val <- NULL
