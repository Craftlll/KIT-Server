# -----------------------------------------------------------------------------
# 脚本名称: NO14_data_pipeline.R
# 功能: 数据预处理与轻量化转换流水线 (NO14 Multiple 10x Folders Merge)
# -----------------------------------------------------------------------------
...
run_preprocess <- function(input_dir, output_rds) {
    ...
    # Filter only those containing matrix.mtx.gz or matrix.mtx
    sample_dirs <- Filter(function(x) {
        file.exists(file.path(x, "matrix.mtx.gz")) || file.exists(file.path(x, "matrix.mtx"))
    }, subdirs)

    if (length(sample_dirs) == 0) stop("No 10x directories (containing matrix.mtx) found!")
...
if (length(args) < 1) stop("Usage: Rscript NO14_data_pipeline.R [preprocess|convert] ...")
action <- args[1]
input_val <- NULL
output_val <- NULL
