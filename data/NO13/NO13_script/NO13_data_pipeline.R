# -----------------------------------------------------------------------------
# 脚本名称: NO13_data_pipeline.R
# 功能: 数据预处理与轻量化转换流水线 (NO13 Multiple TXT.GZ - Offset Header)
# -----------------------------------------------------------------------------
...
run_preprocess <- function(input_dir, output_rds) {
    ...
    if (length(files) == 0) stop("No .txt.gz files found!")
    ...
    tryCatch(
        {
            ...
            for (f in files) {
                # Sample Name: "GSM5222730_s200525a4.expression_matrix.txt.gz" -> "s200525a4"
                s_name <- basename(f)
                s_name <- gsub("^GSM[0-9]+_", "", s_name)
                s_name <- gsub("\\.expression_matrix\\.txt\\.gz$", "", s_name)

                flog.info(paste("Reading sample:", s_name, "from", basename(f)))

                # Handling Offset Header
                # 1. Read first line as headers (Cell Names)
                headers <- scan(f, nlines = 1, what = character(), quiet = TRUE)

                # 2. Read Data (Skip 1). Header=FALSE
                # This ensures we get all columns.
                # Col 1 will be Genes. Cols 2..N+1 will be Counts.
                # Check dimensions match

                raw_data <- fread(f, skip = 1, header = FALSE)

                if (ncol(raw_data) == length(headers) + 1) {
                    # Good: first col is genes, rest are cells
                    gene_names <- as.character(raw_data[[1]])
                    raw_mat <- as.matrix(raw_data[, -1])
                    colnames(raw_mat) <- headers
                } else if (ncol(raw_data) == length(headers)) {
                    # Ambiguous: maybe first column IS missing in data?
                    # Or headers INCLUDED gene column name?
                    # Based on inspection, we saw 2307 headers vs 2308 columns.
                    # So Case 1 applies.
                    flog.warn("Unexpected column count match. Assuming standard matrix.")
                    gene_names <- as.character(raw_data[[1]]) # Assume first col is still genes
                    raw_mat <- as.matrix(raw_data[, -1])
                    if (length(headers) > ncol(raw_mat)) {
                        # Headers likely include Gene label?
                        colnames(raw_mat) <- headers[-1]
                    } else {
                        colnames(raw_mat) <- headers
                    }
                } else {
                    stop(paste("Column mismatch: Headers", length(headers), "vs Data", ncol(raw_data)))
                }
...
if (length(args) < 1) stop("Usage: Rscript NO13_data_pipeline.R [preprocess|convert] ...")
action <- args[1]
input_val <- NULL
output_val <- NULL
