# -----------------------------------------------------------------------------
# 脚本名称: NO7_data_pipeline.R
# 功能: 数据预处理与轻量化转换流水线 (NO7 TXT.GZ Merge)
# -----------------------------------------------------------------------------
    ...
    tryCatch(
        {
            ...
            for (f in files) {
                # 提取 sample name
                # Example: GSM2949336_74_NLW44_A_YesVegf.txt.gz -> 74_NLW44_A_YesVegf
                s_name <- gsub("^GSM[0-9]+_", "", basename(f))
                s_name <- gsub("\\.txt\\.gz$", "", s_name)

                flog.info(paste("Reading sample:", s_name, "from", basename(f)))

                # 使用 data.table::fread 读取
...
if (length(args) < 1) stop("Usage: Rscript NO7_data_pipeline.R [preprocess|convert] ...")
action <- args[1]
input_val <- NULL
output_val <- NULL
