# -----------------------------------------------------------------------------
# 脚本名称: NO11_data_pipeline.R
# 功能: 数据预处理与轻量化转换流水线 (NO11 Single TXT with Metadata Row)
# -----------------------------------------------------------------------------
...
run_preprocess <- function(input_dir, output_rds) {
    ...
    if (length(files) == 0) stop("No .txt files found in input directory!")
    if (length(files) > 1) flog.warn("Multiple .txt files found, utilizing the first one.")

    target_file <- files[1]

    tryCatch(
        {
            s_name <- "NO11_MouseKidney"
            flog.info(paste("Reading file:", basename(target_file)))

            # 读取
            raw_data <- fread(target_file, header = TRUE)

            # Step 1: Extract Metadata Row (Row 1: Cluster_Number)
            # The first column of Row 1 is "Cluster_Number" (or whatever name)
            # The rest are cluster IDs for cells

            # Check if Row 1 is metadata
            first_col_name <- as.character(raw_data[1, 1])
            flog.info(paste("First row, first col:", first_col_name))

            cluster_info <- NULL
            if (grepl("Cluster", first_col_name, ignore.case = TRUE)) {
                flog.info("Detected Metadata Row (Cluster Info). Extracting...")
                cluster_info <- as.character(raw_data[1, -1])
                # Remove Row 1 from data
                raw_data <- raw_data[-1, ]
            } else {
                flog.warn("Did not detect 'Cluster' in first row. Proceeding without metadata extraction.")
            }
            ...
            if (any(duplicated(gene_names))) {
                flog.warn("Duplicate genes found - resolving...")
            ...
            if (mean(obj[["percent.mt"]][, 1]) < 0.1) {
                # Try lower case mt- if upper caese yielded low
                obj[["percent.mt"]] <- PercentageFeatureSet(obj, pattern = "^mt-")
            }

            # Add Cluster Info if extracted
            if (!is.null(cluster_info)) {
                # Ensure length matches
                if (length(cluster_info) == ncol(obj)) {
                    obj$original_cluster <- cluster_info
                    flog.info("Added 'original_cluster' metadata.")
                } else {
                    flog.warn(paste("Cluster info length (`", length(cluster_info), "`) does not match cell count (`", ncol(obj), "`). Skipping."))
                }
            }

            flog.info(paste("Total cells:", ncol(obj)))
    ...
    tryCatch(
        {
            ...
            keep_cols <- c("orig.ident", "nCount_RNA", "nFeature_RNA", "percent.mt", "cell_type", "CD45_status", "UMAP_1", "UMAP_2", "seurat_clusters", "CellID", "original_cluster")
...
if (length(args) < 1) stop("Usage: Rscript NO11_data_pipeline.R [preprocess|convert] ...")
action <- args[1]
input_val <- NULL
output_val <- NULL
