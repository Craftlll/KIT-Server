# -----------------------------------------------------------------------------
# 脚本名称: NO8_data_pipeline.R
# 功能: 数据预处理与轻量化转换流水线 (NO8 10x Folders Merge)
# -----------------------------------------------------------------------------
...
# ==============================================================================
# 模块 A: 预处理 (Multiple 10x Folders -> RDS)
# ==============================================================================
run_preprocess <- function(input_dir, output_rds) {
    ...
    # 寻找子目录 (Assume any dir with 'matrix.mtx.gz' is a sample)
    # Find all subdirectories
    subdirs <- list.dirs(input_dir, recursive = FALSE, full.names = TRUE)
    # Filter only those containing matrix.mtx.gz (checking for 10x format)
    sample_dirs <- Filter(function(x) file.exists(file.path(x, "matrix.mtx.gz")), subdirs)

    if (length(sample_dirs) == 0) {
        # Try checking recursive depth 1 if not found in root
        subdirs <- list.dirs(input_dir, recursive = TRUE, full.names = TRUE)
        sample_dirs <- Filter(function(x) file.exists(file.path(x, "matrix.mtx.gz")), subdirs)
    }

    if (length(sample_dirs) == 0) stop("No 10x directories (containing matrix.mtx.gz) found!")
    ...
    tryCatch(
        {
            ...
            for (d in sample_dirs) {
                # Extract sample name
                # Example: /path/to/GSM6046006_A001 -> A001 or GSM6046006_A001
                s_name <- basename(d)
                flog.info(paste("Reading sample:", s_name, "from", d))
...
if (length(args) < 1) stop("Usage: Rscript NO8_data_pipeline.R [preprocess|convert] ...")
action <- args[1]
input_val <- NULL
output_val <- NULL
